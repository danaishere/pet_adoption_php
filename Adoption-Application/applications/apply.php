...
if (isset($_POST['apply'])) {
    ...
    $check = $conn->query("SELECT id FROM applications WHERE adopter_id=$user_id AND pet_id=$pet_id AND status != 'rejected'");
    if ($check->num_rows > 0) {
        header("Location: success.php?msg=already");
        exit();
    }
    $sql = "INSERT INTO applications (adopter_id, pet_id, status, interview_date, notes) VALUES ($user_id, $pet_id, 'pending', NULL, NULL)";
    if ($conn->query($sql)) {
    ...
    <div class="col-md-4">
        <div class="card shadow-sm h-100">
            <?php if (!empty($pet['photo'])): ?>
                <img src="/Adoption-Application/<?= htmlspecialchars($pet['photo']) ?>"
            ...
            <div class="card-body d-flex flex-column">
                ...
                <p class="card-text text-muted small mt-1"><?= htmlspecialchars($pet['description'] ?? '') ?></p>
                <div class="mt-auto">
